...
# Recommended: editable install with dev extras (pytest / ruff / mypy)
pip install -e .[dev]

# Alternative (older pip-only flow):
# pip install -r requirements.txt   # note: this uses the loose version pins
...
**Dependencies**: `openai>=1.30,<2.0`, `pydantic>=2.5,<3.0` (canonical source: [`pyproject.toml`](pyproject.toml))
**Python Version**: ≥ 3.10

> Recommended install: `pip install -e .[dev]` (gives you pytest / ruff / mypy).
> `requirements.txt` is a legacy-compatibility subset—its version pins are intentionally **looser**.

> **First run after clone**: `character_data/` ships empty. Running `python app.py` drops you into an interactive menu that lets you create your first character; you can also run `python app.py --list` to see registered characters at any time.
