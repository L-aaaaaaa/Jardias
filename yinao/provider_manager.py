...
from data_shape import IPUProviderConfig, IPUConfigFile
...
class IPURegistry:
    ...
    def _coerce_provider_ipus(cls, provider_data: dict) -> dict:
        ...
        if "ipus" in provider_data:
            provider_data["ipus"] = {
                k: cls._coerce_ipu_entry(v) for k, v in provider_data["ipus"].items()}
    ...
    def load(self) -> IPUConfigFile:
        """加载配置（首次从文件，后续返回缓存）"""
        # 锁外快速路径：已加载则直接返回，避免每次都抢锁
        if self._providers is not None: return self._providers
        with self._lock:
            if self._providers is not None: return self._providers  # 锁内再次检查，python特性
            if not self.config_path.exists():
                self._providers = IPUConfigFile()
                self._save_nolock(self._providers)
            else:
                self._providers = IPUConfigFile(**self._read_raw())
    ...
    def save(self):
        """持久化当前内存配置到文件"""
        with self._lock:
            if self._providers is not None: self._save_nolock(self._providers)
        ...
        with self._lock:
            ...
            for p in cfg.providers:
                if p.name == provider_name:
                    if short_name in p.ipus: raise ValueError(f"智能基元简称 {short_name} 已存在")
                    entry = {"id": ipu_id}
    ...
    def update_ipu(self, provider_name: str, short_name: str, new_ipu_id: str, caps: list[str] | None = None):
        with self._lock:
            ...
            for p in cfg.providers:
                if p.name == provider_name:
                    if short_name not in p.ipus: raise ValueError(f"智能基元简称 {short_name} 不存在")
                    entry = {"id": new_ipu_id}
