"""yinao/ — IPU 解析与 provider 配置加载。

真实 API：
- IPURegistry(config_path=...).load() / reload() / get_provider(name)
- IPUVendor (enum)
...
from yinao.provider_manager import IPURegistry
...
# ── IPURegistry 单例 + reload ──────────────────────────

class TestIPURegistry:
    ...
    def test_load(self, tmp_path):
        ...
        mgr = IPURegistry(config_path=str(p))
        cfg = mgr.load()
    ...
    def test_add_ipu_persists(self, tmp_path):
        ...
        mgr.save()
        # 重读
        mgr2 = IPURegistry(config_path=str(p))
